...
static XLTieBarLoading *loading = nil;
+(instancetype)sharedLoading{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        loading = [[self alloc] initWithFrame:[ZWHelpUtil getCurrentViewController].view.bounds];
    });
    return loading;
}
...
-(void)buildUI
{
    ...
    _imageView1.backgroundColor = [UIColor colorWithHexString:COLR_MAIN];
    ...
    _imageView2.backgroundColor = [UIColor colorWithHexString:COLR_MAIN];
...
-(void)showInView{
    [self hideInView];
    [[ZWHelpUtil getCurrentViewController].view addSubview:loading];
    [loading show];
}
